...
The complitementary design documents [VNF Deployment](#osc-vnf-deployment) and [Traffic Redirection](#osc-traffic-redirection) outline the details of how OSC will deploy the containerized VNF and apply the security policies repectively.
...
```java
NeworkElement getNetworkElement(String deviceOwnerId)
// The provided ID SHOULD be the K8s Pod UID. The returned network element will container vport id, mac address, ip address and parent id (domain id)
```

> **Restriction**: As mentioned above the provider deviceOwnerId should be the pod id. However due to an API limitation in Nuage as an interim workaround the pod name + namespace will be passed instead. Once the Nuage API supports fecthing the port using the pod ID this workaround should be reverted.
...
These APIs allow a client to receive live notification of the filtered objects on Kubernetes. Notifications arrive on an `eventReceived` method through **Fabric8** while the running thread is alive.  This will likely require one instance of the watcher per security group member given that inclusive filtering (`OR`) is not supported by K8s APIs or Fabric8.
Below we can see a code snippet of this API:
...
> Note: this API will return an empty collection in case no pod is found with any of the provided label.
...
* * **UpdateOrDeleteK8ssSecurityGroupMetaTask:** The main purpose of this task is to trigger the task `KubernetesSecurityGroupLabelCheckMetTask` for each label security group member. Similarly to the existing `SecurityGroupUpdateOrDeleteMetatask` this task will also add the existing task `PortGroupCheckMetaTask` if the SDN controller supports port group.

* **CheckK8sSecurityGroupLabelMetaTask:** This metatask forks the graph in two possible options for a given label security member: update or deletion, adding to the graph either `SecurityGroupMemberLabelUpdateMetaTask` or;
