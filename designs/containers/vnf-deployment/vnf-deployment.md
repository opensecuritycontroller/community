...
The Nuage SDN currently requires the pod VNFs to be present on the same namespace as the protected workloads.
...
swVersion (string),
virtualizationType (string) = ['OPENSTACK', 'KUBERNETES'],
virtualizationVersion (string),
imageUrl (string),                            // For container VNFs this should be the Docker image name*.
