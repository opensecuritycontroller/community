...
In this article you can find information how is [OSGi](osgi.md) integrated into project, and how to avoid and handle some typical OSGi troubles related to development. 
...
Every project mentioned in previous paragraph has `bnd.bnd` file inside which treats `pom.xml` like a repository. 

### Adding/Including dependencies to uber bundle
...
First of all, if we want to include resources based on `pomx.ml` to our bundle we need to have that line in our `bnd.bnd` file:
...
*Notice: this is not the best way to work with OSGi, but not all jars are OSGi compatible!*

#### Import/Export external classes to our bundle

There are situations, when we include some resource to our bundle, but we don't want to use all classes from that jar. Then we are able to exclude package using negation mark. Let us consider `osc-uber\bnd.bnd` file again:
...
It is easy now to see that if we want to import some package from bundle injected directly into OSGi container, we just don't use negation mark:
...
*Notice: if you will use some class i. e. `com.fasterxml.jackson.annotation.JsonIgnore;`, which is from bundle injected into OSGi container and forget to import `com.fasterxml.jackson.annotation`,  annotation `@JsonIgnore` won't work*

To export services/classes outside our bundle, we use this line in `bnd.bnd` file:
...
## Handle Resolve Exception Problems

#### Missing requirement while adding new bundle to OSGi Container

This is a typical situation while adding new bundle to OSGi. Reason is very simple - your bundle requires some other bundle in OSGi Container to work and you need to provide it or them.
...
3. We can see that two bundles are not active becoese something is missing when looking at log file:
...
5. Look at console log:
   ...
   So you can see that feshly added bundle `com.fasterxml.jackson.jaxrs.jackson-jaxrs-json-provider;version='[2.8.5,2.8.6)',\` is in our Container, but it is missing some dependecy that also is missed in `osc-uber` and `jersey-media-json-jackson` like before in step 2
...
*Notice: to know exactly what dependency should be added to OSGi Container while adding some new one, you need to look at dependency network of that bundle and provide all that are require.*

#### Missing requirement while adding dependency to Uber bundle

This situation gives us same log as that one before, but our intend is different. We want to include resource inside `osc-uber` bundle.
...
#### Missing requirement that we do not want to have in runtime

In some situation, we would like to include some library but it's dependency force us to include also some other lib to our uber bundle. In this situation we can exclude packages.
